// ANCESTRY_FILE Imported from http://eloquentjavascript.net/code/ancestry.js
...
var CENTURY_MAP = createCenturyTable(ANCESTRY);
console.debug("CENTURY_MAP:", CENTURY_MAP);
...
function createCenturyTable(ancestry)
{
    let workingTable = {};
    
    ancestry.forEach(function(ancestor) {
        let index = inferCenturyOf(ancestor);
    
        if (workingTable[index] === undefined)
            workingTable[index] = [];
        
        workingTable[index].push(ancestor.name);
    });

    return workingTable;
}

function calcAverageAge(ancestry, personNames)
{
    let sumAges =
        personNames
            .map(
                function(personName)
                {
                    return findPersonByName(ancestry, personName);
                }
            )
            .reduce(
                function(ageAcc, person)
                {
                    return ageAcc + inferAge(person);
                },
                0
            );

    return sumAges / personNames.length;
}
...
function getYearBorn(person)
{
    return person.born;
}

function getYearDied(person)
{
    return person.died;
}

function inferCenturyOf(person)
{
    return Math.ceil(getYearDied(person) / 100);
}

function inferAge(person)
{
    return getYearDied(person) - getYearBorn(person);
}
...
function main()
{
    // Create table of average age per century
    
    let averageAgePerCentury = {};
    for (var century in CENTURY_MAP)
    {
        let names = CENTURY_MAP[century];
        averageAgePerCentury[century] = calcAverageAge(ANCESTRY, names);
    }

    // Print

    for (var averageAge in averageAgePerCentury)
    {
        var averageAge = averageAgePerCentury[averageAge];
        
        console.log(
            `Average age in century ${century} is`,
            averageAge
        );
    }
}
