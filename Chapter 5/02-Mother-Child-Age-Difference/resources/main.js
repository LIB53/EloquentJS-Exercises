// Imported from http://eloquentjavascript.net/code/ancestry.js
...
function main()
{
    console.debug("Ancestry:", ANCESTRY);


    // Get usable ancestors

    console.info("Filtering for ancestors with available mother data...");

    let factorableAncestors = ANCESTRY.filter(
        function(ancestor)
        {
            return findPersonByName(ANCESTRY, getMotherName(ancestor)) !== undefined;
        }
    );

    console.debug("Ancestors with available mother data:", factorableAncestors);

    // TODO: cache findPersonByName results,
    //       or replace filter -> map with C like routine.

    // Get list of mother ages

    console.info("Compiling list of mother ages...");
    
    let availableMotherAgesAtBirths = factorableAncestors.map(
        function(ancestor)
        {
            let mother = findPersonByName(ANCESTRY, getMotherName(ancestor));
            let motherAgeAtAncestorBirth = CalcAgeAtYear(
                getBirthYear(mother),
                getBirthYear(ancestor)
            );

            return motherAgeAtAncestorBirth;
        }
    )

    console.debug("Ages of available mothers:", availableMotherAgesAtBirths);

    // Average mother ages at births

    console.info("Calcuating average...");

    let sumMotherAgeAtBirths = availableMotherAgesAtBirths.reduce(
        function(ageAcc, age) { return ageAcc + age; }
    );

    console.debug("sumMotherAgeAtBirths:", sumMotherAgeAtBirths);

    let averageMotherAgeAtBirths =
        sumMotherAgeAtBirths / availableMotherAgesAtBirths.length;

    console.debug("averageMotherAgeAtBirths:", averageMotherAgeAtBirths);
    
    console.log(
        "Average age of mothers at birth of each ancestor:",
        averageMotherAgeAtBirths
    );
}


function findPersonByName(ancestry, personName)
{
    let candidates = ancestry.filter(
        function(ancestor)
        {
            if (ancestor.name == personName)
              return true;
        }
    );
    ...
    if (candidates.length > 1)
        console.warn("More than one person found with name " + personName);
...
function CalcAgeAtYear(personBirthYear, year)
{
